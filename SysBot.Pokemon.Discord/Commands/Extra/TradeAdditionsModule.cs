...
namespace SysBot.Pokemon.Discord
{
    ...
    public class TradeAdditionsModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        [Summary("Initialize posting of SV shiny result embeds to specified Discord channels.")]
        [RequireSudo]
        public async Task InitializeEmbedsSVO()
        {
            if (SysCord<T>.Runner.Hub.Config.StopConditions.ResultsEmbedChannels == string.Empty)
            {
                await ReplyAsync("No channels to post embeds in.").ConfigureAwait(false);
            ...
            await ReplyAsync(!OverworldBotSV.EmbedsInitialized ? "Scarlet | Violet Overworld Embed task started!" : "Scarlet | Violet Overworld Embed task stopped!").ConfigureAwait(false);
            if (OverworldBotSV.EmbedsInitialized)
                OverworldBotSV.EmbedSource.Cancel();
            else _ = Task.Run(async () => await SVOEmbedLoop(channels));
            OverworldBotSV.EmbedsInitialized ^= true;
        }

        private async Task SVOEmbedLoop(List<ulong> channels)
        {
            var ping = SysCord<T>.Runner.Hub.Config.StopConditions.MatchFoundEchoMention;
            while (!OverworldBotSV.EmbedSource.IsCancellationRequested)
            {
                if (OverworldBotSV.EmbedMon.Item1 != null)
                {
                    var url = TradeExtensions<PK9>.PokeImg(OverworldBotSV.EmbedMon.Item1, false, false);
                    var is1of100 = (Species)OverworldBotSV.EmbedMon.Item1.Species is Species.Dunsparce or Species.Tandemaus;
                    var spec = string.Empty;
                    if (is1of100)
                    {
                        if (OverworldBotSV.EmbedMon.Item1.EncryptionConstant % 100 == 0)
                        {
                            if ((Species)OverworldBotSV.EmbedMon.Item1.Species is Species.Dunsparce)
                                spec = "\n3 Segment";
                            else
                                spec = "\nFamily of 3";
                        }
                    ...
                    var markurl = string.Empty;
                    if (OverworldBotSV.EmbedMon.Item2)
                        markurl = "https://i.imgur.com/T8vEiIk.jpg";
                    else
                        markurl = $"https://i.imgur.com/t2M8qF4.png";

                    bool hasMark = StopConditionSettings.HasMark(OverworldBotSV.EmbedMon.Item1, out RibbonIndex mark);
                    string msg = hasMark ? $"{mark.ToString().Replace("Mark", "")}mark" : "";
                    if (hasMark)
                    {
                        markurl = $"https://www.serebii.net/swordshield/ribbons/" + $"{msg.ToLower()}" + ".png";
                    ...
                    if (mark == RibbonIndex.MarkMini)
                        markEntryText = "The Teeny";
                    if (mark == RibbonIndex.MarkJumbo)
                        markEntryText = "The Great";

                    var gender = OverworldBotSV.EmbedMon.Item1.Gender == 0 ? " - (M)" : OverworldBotSV.EmbedMon.Item1.Gender == 1 ? " - (F)" : "";
                    var print = $"{(OverworldBotSV.EmbedMon.Item1.ShinyXor == 0 ? "■ - " : OverworldBotSV.EmbedMon.Item1.ShinyXor <= 16 ? "★ - " : "")}{SpeciesName.GetSpeciesNameGeneration(OverworldBotSV.EmbedMon.Item1.Species, 2, 9)}{TradeExtensions<PK9>.FormOutput(OverworldBotSV.EmbedMon.Item1.Species, OverworldBotSV.EmbedMon.Item1.Form, out _)}{markEntryText}{gender}\nIVs: {OverworldBotSV.EmbedMon.Item1.IV_HP}/{OverworldBotSV.EmbedMon.Item1.IV_ATK}/{OverworldBotSV.EmbedMon.Item1.IV_DEF}/{OverworldBotSV.EmbedMon.Item1.IV_SPA}/{OverworldBotSV.EmbedMon.Item1.IV_SPD}/{OverworldBotSV.EmbedMon.Item1.IV_SPE}";
                    var description = print + $"\nScale: {PokeSizeDetailedUtil.GetSizeRating(OverworldBotSV.EmbedMon.Item1.Scale)}\n{spec}";

                    var author = new EmbedAuthorBuilder { IconUrl = markurl, Name = OverworldBotSV.EmbedMon.Item2 ? "Match found!" : "Unwanted match..." };
                    var embed = new EmbedBuilder
                    {
                        Color = OverworldBotSV.EmbedMon.Item2 ? Color.Teal : Color.Red,
                        ThumbnailUrl = url
                    }.WithAuthor(author).WithDescription(description);
                    ...
                    foreach (var guild in guilds)
                    {
                        foreach (var channel in channels)
                        {
                            ...
                            if (ch != default && ch is ISocketMessageChannel sock)
                            {
                                try
                                {
                                    await sock.SendMessageAsync(OverworldBotSV.EmbedMon.Item2 ? ping : "", embed: embed.Build()).ConfigureAwait(false);
                                }
                                catch { }
                            }
                        }
                    }
                    OverworldBotSV.EmbedMon.Item1 = null;
                }
                else await Task.Delay(1_000).ConfigureAwait(false);
            }
            OverworldBotSV.EmbedSource = new();
        }
